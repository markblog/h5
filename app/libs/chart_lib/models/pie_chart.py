...
class PieChart(BaseChart):
    """
    PieChart is the class of displaying pie chart.
    ...
    def __init__(self):
        super().__init__()
        self.__type__ = 'pie'
        # self.__description__ = 'pie chart'

    def _available_types(self):
        self.available_types = ['pie']
    ...
    def transformation(self, raw_data):
        ...
        for index, row in df.iterrows():
        	...
        	if index == 0:
        		...
        		series.append({'name': row[x_label], 'y': float(row[y_label])})
    ...
    def _get_hover_data(self, raw_data, df, x_label, y_count = 0):
        ...
        if highlight:
            for i in range(len(df_highlights)):
                ...
                index = 0
                for y_name in self.__series__[0].get('data'):
